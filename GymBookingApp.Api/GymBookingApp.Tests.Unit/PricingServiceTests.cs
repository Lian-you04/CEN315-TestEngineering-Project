...
public class PricingServiceTests
{
    ...
    [InlineData(100, MembershipType.Standard, 0.8, 110)]
    
    [InlineData(100, MembershipType.Student, 0.85, 88)]
    
    [InlineData(100, MembershipType.Premium, 0.9, 99)]
    
    [InlineData(100, MembershipType.Standard, 0.79, 100)]
    ...
    public bool Price_Should_Never_Be_Negative(double basePriceRaw, int membershipTypeRaw, double occupancyRaw)
    {
        ...
        MembershipType type = (MembershipType)(Math.Abs(membershipTypeRaw) % 3);
    ...
    [Fact]
    public void CalculatePrice_Performance_ShouldBeFast()
    {
        var watch = System.Diagnostics.Stopwatch.StartNew();

        for (int i = 0; i < 1000; i++)
        {
            _pricingService.CalculatePrice(100m, MembershipType.Student, 0.5);
        }

        watch.Stop();
        Assert.True(watch.ElapsedMilliseconds < 500, "Performans kriteri aşıldı!");
    }
}